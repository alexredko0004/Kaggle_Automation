...
test('Verify that images on home page correspond to screenshots', async ({ page }) => {
  ...
  await test.step('Verify that main menu icons correspond to screenshots',async()=>{
    ...
    const modelsIcon = page.locator("i[title='Models']");
    const codeIcon = page.locator("i[title='Code']");
    const discussionsIcon = page.locator("i[title='Discussions']");
    const learnIcon = page.locator("i[title='Learn']");
    const yourWorkIcon = page.locator("i[title='Your Work']")
    const viewActiveEventsIcon = page.getByLabel('Open Active Events dialog').nth(1);
